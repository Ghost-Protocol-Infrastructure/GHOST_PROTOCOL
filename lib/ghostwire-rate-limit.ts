...
type GhostWireRateLimitAction = "artifacts";
...
const LIMITS_PER_WINDOW: Record<GhostWireRateLimitAction, number> = {
  artifacts: 20,
};
...
export const consumeGhostWireRateLimit = (input: {
  request: NextRequest;
  action: GhostWireRateLimitAction;
  jobId: string;
  ...
  const actor = (input.actorAddress ?? "anonymous").toLowerCase();
  const key = `${input.action}:${input.jobId}:${actor}:${ip}`;
  ...
  if (existing.count >= limit) {
    ...
    return {
      ...
      error: `Too many GhostWire artifact submissions. Try again in ${retryAfterSeconds}s.`,
      retryAfterSeconds,
    };
  }
