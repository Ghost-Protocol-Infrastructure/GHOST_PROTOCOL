...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const signerAddress = normalizeOwnerAddress(payload.signerAddress);
  const label = normalizeDelegatedSignerLabel(payload.label);
  ...
  if (!signerAddress) return json({ code: 400, error: "signerAddress must be a valid 0x address." }, 400);
  if (payload.label != null && !label) {
    return json({ code: 400, error: "label must be printable ASCII and 64 characters or fewer." }, 400);
  }
  if (!serviceSlug) {
  ...
  try {
    ...
    if (actorAddress !== canonicalOwner) {
      return json(
        {
          code: 403,
          error: "actorAddress must match the indexed agent owner for delegated signer registration.",
          expectedOwnerAddress: canonicalOwner,
        },
        403,
      );
    ...
    const activeSignerCount = await prisma.agentGatewayDelegatedSigner.count({
      where: { gatewayConfigId: config.id, status: "ACTIVE" },
    });
    if (activeSignerCount >= AGENT_GATEWAY_MAX_ACTIVE_DELEGATED_SIGNERS) {
      const signers = await loadGatewaySigners(config.id);
      return json(
        {
          code: 409,
          error: "Maximum active delegated signers reached. Revoke an existing signer before registering a new one.",
          authMode: "wallet-signature",
          maxActiveSigners: AGENT_GATEWAY_MAX_ACTIVE_DELEGATED_SIGNERS,
          activeSignerCount,
          signers: signers.map(toDelegatedSignerResponse),
          errorCode: "MAX_ACTIVE_SIGNERS_EXCEEDED",
        },
        409,
      );
    }
    ...
    const signers = await loadGatewaySigners(config.id);
    const nextActiveSignerCount = signers.filter((row) => row.status === "ACTIVE").length;
    ...
    return json(
      {
        ...
        authMode: "wallet-signature",
        maxActiveSigners: AGENT_GATEWAY_MAX_ACTIVE_DELEGATED_SIGNERS,
        activeSignerCount: nextActiveSignerCount,
        signers: signers.map(toDelegatedSignerResponse),
      },
      200,
    );
    ...
    return json({ code: 500, error: "Failed to register delegated signer." }, 500);
  }
}
